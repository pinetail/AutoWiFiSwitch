...
public class GeoHex {
	...
	public static final class Zone {
		...
		/** 北 方向へ distance(Hex の数) 移動した Zone を返します。 */
		public Zone offsetToNorth(int distance) {
			return GeoHex.getZoneByXY(this.x + distance, this.y + distance, this.level);
		}

		/** 北東 方向へ distance(Hex の数) 移動した Zone を返します。 */
		public Zone offsetToNorthEast(int distance) {
			return GeoHex.getZoneByXY(this.x + distance, this.y, this.level);
		}

		/** 南東 方向へ distance(Hex の数) 移動した Zone を返します。 */
		public Zone offsetToSouthEast(int distance) {
			return GeoHex.getZoneByXY(this.x, this.y - distance, this.level);
		}

		/** 南 方向へ distance(Hex の数) 移動した Zone を返します。 */
		public Zone offsetToSouth(int distance) {
			return GeoHex.getZoneByXY(this.x - distance, this.y - distance, this.level);
		}

		/** 南西 方向へ distance(Hex の数) 移動した Zone を返します。 */
		public Zone offsetToSouthWest(int distance) {
			return GeoHex.getZoneByXY(this.x - distance, this.y, this.level);
		}

		/** 北西 方向へ distance(Hex の数) 移動した Zone を返します。 */
		public Zone offsetToNorthWest(int distance) {
			return GeoHex.getZoneByXY(this.x, this.y + distance, this.level);
		}

		/**
		 * 現在の GeoHex を六方向に膨らました GeoHex 群を返します。
		 *
		 * @param distance 膨らます量(Hex の数)。
		 * @return 膨らんだ GeoHex 群 。
		 * @author Hironobu OKUYAMA
		 */
		public List<Zone> inflate(int distance) {

			List<Zone> zones = new Vector<Zone>();

			Zone zone = GeoHex.getZoneByCode(this.code); // 自分
			zones.add(zone);

			for (int j = 1; j <= distance; j++) {

				zone = zone.offsetToNorth(1); // 上へ

				for (int i = 0; i < j; i++) {
					zone = zone.offsetToSouthEast(1); zones.add(zone);  // 右下へ
				}
				for (int i = 0; i < j; i++) {
					zone = zone.offsetToSouth(1); zones.add(zone);     // 下へ
				}
				for (int i = 0; i < j; i++) {
					zone = zone.offsetToSouthWest(1); zones.add(zone); // 左下へ
				}
				for (int i = 0; i < j; i++) {
					zone = zone.offsetToNorthWest(1); zones.add(zone); // 左上へ
				}
				for (int i = 0; i < j; i++) {
					zone = zone.offsetToNorth(1); zones.add(zone);     // 上へ
				}
				for (int i = 0; i < j; i++) {
					zone = zone.offsetToNorthEast(1); zones.add(zone); // 右上へ
				}
			}
		...
		public boolean intersects(double x, double y, double radius) {
			...
			return hexPolygon.intersects(bufferedPoint);
		}
	}
